...
# 1. Regenerate catalogo_assets.html with ALL 35 items including the new map
...
for filename in files:
    ...
    # Custom naming for newly added
    is_new = False
    name_label = f"Imagen #{valid_id:02d}"
    if "mapa" in filename.lower():
        is_new = True
        name_label = "Mapa Masterplan Posadas (con Pin Torre Edmon)"
    elif "planta" in filename.lower():
        is_new = True
        name_label = "Planta Oficial de Distribución"

    items.append({
        "id": f"{valid_id:02d}",
        "webp": f"assets/catalog_previews/render_{valid_id:02d}.webp" if os.path.exists(f"assets/catalog_previews/render_{valid_id:02d}.webp") else f"assets/raw_pure_extracted/{filename}",
...
# Add user's new planta if not yet in raw
if os.path.exists("assets/plantas render.png") and not any("plantas render" in x["raw"] for x in items):
...
<body>
  ...
  <div class="top-bar">
    <div class="title-area">
      ...
      <p>Haz clic en cada imagen para delimitar el recorte puro y asignar su categoría. Incluye el nuevo <strong>Mapa Masterplan con Pin</strong>.</p>
    </div>
...
print("catalogo_assets.html actualizado con el nuevo mapa en catalogo!")

# 2. Update index.html to use the new masterplan map path directly
with open("index.html", "r", encoding="utf-8") as f:
    site_html = f.read()

# Replace any old map path with mapa_ubicacion_masterplan.webp
site_html = site_html.replace("assets/curated/exterior/mapa_ubicacion.webp", "assets/curated/exterior/mapa_ubicacion_masterplan.webp")

with open("index.html", "w", encoding="utf-8") as f:
    f.write(site_html)

print("index.html actualizado apuntando a assets/curated/exterior/mapa_ubicacion_masterplan.webp!")
