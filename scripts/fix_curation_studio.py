...
<body>
  ...
  <div class="top-bar">
    <div class="title-area">
      ...
      <p>Haz clic en cada imagen para <strong>delimitar el recorte puro y asignar su categoría</strong>. Al guardar, se registra y se cierra para continuar con la siguiente.</p>
    </div>
    <div class="top-actions">
      <button class="btn btn-secondary" onclick="exportData()"><i class="fas fa-download"></i> Descargar Registro JSON</button>
  ...
  <div class="instructions-card">
    ...
    <div>
      <strong>Flujo de trabajo ágil:</strong><br>
      1. Haz clic en una imagen de la grilla.<br>
      2. Ajusta el recuadro para remover marcos o textos laterales.<br>
      3. Selecciona la categoría (Exterior, Amenities, Interiores, Planta, Descartar) y haz clic en <strong>"Guardar Cambios y Continuar"</strong>.<br>
      4. El modal se cerrará automáticamente y podrás continuar con el resto sin interrupciones.
    </div>
  </div>

  <div class="stats-bar">
    <div class="stat-pill"><i class="fas fa-images"></i> Total: <strong>{len(items)} imágenes</strong></div>
    <div class="stat-pill"><i class="fas fa-check-circle" style="color:var(--success)"></i> Registradas para Reprocesar: <strong id="stat-classified">0</strong></div>
  ...
  <!-- Change Record / Log Table Section -->
  <div class="log-section">
    <div class="log-header">
      <h3><i class="fas fa-clipboard-list"></i> Registro de Cambios Guardados para Reprocesamiento</h3>
      <button class="btn btn-danger" onclick="clearAllData()"><i class="fas fa-trash-alt"></i> Limpiar Registro</button>
    </div>
      ...
      <table class="log-table">
        <thead>
          <tr>
            ...
            <th>Área de Recorte (X, Y, Ancho, Alto)</th>
            <th>Estado</th>
          </tr>
        </thead>
        <tbody id="log-table-body">
          <tr><td colspan="6" style="text-align:center; padding:2rem;">Aún no has registrado cambios. Haz clic en cualquier imagen arriba para comenzar.</td></tr>
        </tbody>
      </table>
    </div>
  </div>
  ...
  <script>
    ...
    function renderGrid() {{
      ...
      ASSETS_DATA.forEach(item => {{
        ...
        if (hasSaved) {{
          ...
          const cropInfo = saved.cropData ? `${{Math.round(saved.cropData.width)}}x${{Math.round(saved.cropData.height)}} (x:${{Math.round(saved.cropData.x)}}, y:${{Math.round(saved.cropData.y)}})` : 'Completo';
          tableRows += `
            <tr>
              <td><strong>#${{item.id}}</strong></td>
              <td>${{item.filename}}</td>
              <td><span style="color:var(--gold); font-weight:700;">${{saved.category.toUpperCase()}}</span></td>
              <td>${{saved.label || '-'}}</td>
              <td><code>${{cropInfo}}</code></td>
              <td><span style="color:var(--success); font-weight:600;"><i class="fas fa-check-circle"></i> Listo para Reprocesar</span></td>
            </tr>
          `;
        }}
        ...
        card.innerHTML = `
          ...
          <div class="card-img-wrap">
            <img src="${{hasSaved && saved.cropped_preview ? saved.cropped_preview : item.webp}}" alt="Asset #${{item.id}}" loading="lazy">
          </div>
          <div class="card-body">
            <div>
              <div class="card-title">${{hasSaved && saved.label ? saved.label : (item.is_new ? 'Planta Render' : 'Imagen #' + item.id)}}</div>
      ...
      if (tableRows) {{
        logBody.innerHTML = tableRows;
      }} else {{
        logBody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:2rem;">Aún no has registrado cambios. Haz clic en cualquier imagen arriba para comenzar.</td></tr>';
      }}
    ...
    function openEditor(item) {{
      ...
      imgEl.onload = () => {{
        try {{
          ...
          console.warn("Cropper init fallback:", e);
        }}
      }};
    }}
    ...
    function saveAndCloseCurrent() {{
      ...
      const category = selectedRadio ? selectedRadio.value : 'interiores';
      ...
      if (cropper) {{
        try {{
          cropData = cropper.getData(true); // integer coordinates (x, y, width, height)
          const canvas = cropper.getCroppedCanvas({{ maxWidth: 1200, maxHeight: 800 }});
          if (canvas) {{
            croppedPreview = canvas.toDataURL('image/jpeg', 0.8);
          }}
        }} catch(err) {{
          console.warn("Cropper getData error handled:", err);
        }}
      }}
      ...
      try {{
        ...
        console.warn("LocalStorage full, saving without preview cache:", storageErr);
        curationState[currentItem.id].cropped_preview = null;
        localStorage.setItem('torre_edmon_curation', JSON.stringify(curationState));
      }}
    ...
    function finishAndReprocess() {{
      exportData();
      alert('¡Registro guardado y descargado! Notifícame en el chat para ejecutar el reprocesamiento final automático.');
    }}

    function clearAllData() {{
      if (confirm('¿Estás seguro de que deseas reiniciar el registro de cambios?')) {{
        curationState = {{}};
        localStorage.removeItem('torre_edmon_curation');
        renderGrid();
        showToast('Registro de cambios reiniciado.');
      }}
    }}
...
print("catalogo_assets.html reparado con cierre robusto y sin bloqueos de memoria!")
